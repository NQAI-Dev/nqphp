...
abstract class AbstractTag
{
    ...
    /**
     * Scope tag with a unique ID and compile/attach scoped CSS.
     */
    public function scopedCss(string $css): static
    {
        $scopeId = \Nqphp\Core\View\ScopedCssCompiler::generateScopeId($css);
        $this->setAttribute('data-nq-s', $scopeId);
        $compiled = \Nqphp\Core\View\ScopedCssCompiler::compile($css, $scopeId);
        if ($compiled !== '') {
            $this->children = '<style>' . $compiled . '</style>' . $this->children;
        }
        return $this;
    }

    /**
     * Bind this tag to an external route-scoped CSS ID (generated by RouteAssetManager).
     */
    public function scope(string $scopeId): static
    {
        return $this->setAttribute('data-nq-s', $scopeId);
    }
